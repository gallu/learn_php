...
var_dump($test);
$t2 = filter_input(INPUT_GET, 't2', FILTER_DEFAULT, ['flags' => FILTER_REQUIRE_ARRAY]); // XXX (主にクラックで)スカラ値が与えられたらNULLになる
var_dump($t2);
$t2 = filter_input(INPUT_GET, 't2', FILTER_DEFAULT, ['flags' => FILTER_FORCE_ARRAY]); // XXX (主にクラックで)スカラ値が与えられたら配列にする(その１と同じ挙動)
var_dump($t2);
